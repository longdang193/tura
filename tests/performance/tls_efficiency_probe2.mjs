...
function snapshotStatsFromBody(body) {
  ...
  return {
    ...
    payload_digest: hashText(text), prompt_cache_key_digest: typeof parsed?.prompt_cache_key === "string" ? hashText(parsed.prompt_cache_key) : null,
  };
}
...
function verifyTuraOutput(stdout, workerDiagnostic = "") {
  ...
  const agentText = events.filter((event) => ["agent_message", "assistant_message"].includes(event.item?.type)).map((event) => String(event.item.text || "")).join("\n");
...
async function selfTest() {
  ...
  const assistantOutput = verifyTuraOutput([
    ...Array.from({ length: 10 }, (_, index) => JSON.stringify({ type: "item.completed", item: { type: "command_execution", command: `read-${index}` } })),
    JSON.stringify({ type: "item.completed", item: { type: "assistant_message", text: MARKER } }),
  ].join("\n"));
  if (!assistantOutput.exact_ten_tools || !assistantOutput.final_marker) throw new Error("assistant event self-test failed");
...
async function liveRun(options) {
  ...
  try {
    ...
    proxy = await runtime.startCodexResponsesProxy({ config: runtime.normalizeTokenPilotCodexConfig({ proxyPort, stateDir, upstream: { name: "benchmark-relay", baseUrl: relay.baseUrl, wireApi: "responses" }, proxyMode: { pureForward: false }, modules: { stabilizer: true, reduction: false } }), logger: runtime.createConsoleLogger(false) });
    ...
    for (let repetition = 1; repetition <= options.repetitions; repetition += 1) {
      ...
      for (const arm of order) {
        ...
        const forwardRows = traceRows.filter((row) => ["pure_forward_timing", "pure_forward_cancelled", "pure_forward_failed", "proxy_after_call"].includes(row.stage));
        ...
        const completeForwardRows = forwardRows.filter((row) => row.stage === "pure_forward_timing" || row.stage === "proxy_after_call" || expectedCancellation(row));
